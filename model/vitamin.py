...
class Stem(nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Conv2d(in_chs, out_chs, 3, stride=2, bias=bias,padding=1)
        self.norm1 = norm_act_layer(out_chs)
        # self.conv2 = create_conv2d(out_chs, out_chs, 3, stride=1, bias=bias)

        self.conv2 = nn.Conv2d(out_chs, out_chs, 3, stride=1, bias=bias,padding=1)
...
class MbConvLNBlock(nn.Module):
    ...
    def __init__(
        ...
        self.conv1_1x1 = nn.Conv2d(in_chs, mid_chs, 1, stride=1, bias=True)
        ...
        self.conv2_kxk = nn.Conv2d(mid_chs, mid_chs, kernel_size, stride=stride, dilation=1, groups=mid_chs, bias=True,padding=1)
        # self.conv3_1x1 = create_conv2d(mid_chs, out_chs, 1, bias=True)
        self.conv3_1x1 = nn.Conv2d(mid_chs, out_chs, 1, bias=True)
...
class MbConvStages(nn.Module):
    ...
    def __init__(
            self,
            cfg: VitCfg,
            img_size: Union[int, Tuple[int, int]] = 256, # place holder
            in_chans: int = 3,
    ):
        super().__init__()
        self.grad_checkpointing = False
...
def vitamin_large(pretrained=False, **kwargs) -> VisionTransformer:
    ...
    stage3_args = dict(embed_dim=512, depth=31, num_heads=16, mlp_layer=GeGluMlp, mlp_ratio=2., class_token=False, global_pool='avg')
...
if __name__ == "__main__":
    ...
    model = vitamin_large().to(dev)
    # print(model)
    # print(count_params(model))

    ctran = train_transform()
    image = Image.open('input/dh/testA/0.png').convert('RGB')
    ...
    # x = torch.rand([2,3,224,224]).to(dev)
